 ...
 */
@Slf4j
public class OSSClientSource implements ClientSource, Closeable {

    private final StorageProperty config;
    private Client client;

    public OSSClientSource(StorageProperty storageProperty) {
        // 校验配置合法性
    ...
    public void init() throws ClientSourceException {
        ...
        log.debug("init storage client [{}] ok", ClientType.OSS.name());
    ...
    public void close() throws IOException {
        log.debug("opcooc-storage - shutdown [{}] client", ClientType.OSS.name());
        try {
            client.shutdown();
        } catch (Exception e) {
