 ...
 */
@Slf4j
public class MinIOClientSource implements ClientSource, Closeable {

    private final StorageProperty config;
    private Client client;

    public MinIOClientSource(StorageProperty storageProperty) {
        // 校验配置合法性
    ...
    public void init() throws ClientSourceException {
        ...
        AmazonS3 amazonS3 = AmazonS3ClientBuilder.standard()
                ...
                .withEndpointConfiguration(new AwsClientBuilder.EndpointConfiguration(
                        config.getEndPoint(),
                        ClientType.MINIO.name()))
                .build();

        log.debug("init storage client [{}] ok", ClientType.MINIO.name());
    ...
    public void close() throws IOException {
        log.debug("opcooc-storage - shutdown [{}] client", ClientType.MINIO.name());
        try {
            client.shutdown();
        } catch (Exception e) {
